  ...
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border p-6 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
        ...
        warning: "border-orange-200 bg-orange-50 text-orange-900 dark:border-orange-800 dark:bg-orange-950/50 dark:text-orange-100",
        info: "border-blue-200 bg-blue-50 text-blue-900 dark:border-blue-800 dark:bg-blue-950/50 dark:text-blue-100",
        loading: "border-blue-200 bg-blue-50 text-blue-900 dark:border-blue-800 dark:bg-blue-950/50 dark:text-blue-100"
      }
    },
    defaultVariants: {
      variant: "default"
...
export function ToastProviderComponent({ children }: { children: React.ReactNode }) {
  ...
  const addToast = React.useCallback((toast: Omit<ToastType, "id">) => {
    ...
    // Auto-remove após o tempo definido (padrão: 5 segundos)
  ...
  const updateToast = React.useCallback((id: string, updates: Partial<ToastType>) => {
...
// Funções helper para toasts comuns
export const toast = {
  ...
  loading: (title: string, description?: string) => {
    ...
    return {
      update: (updates: Partial<ToastType>) => {
...
>(({ icon, className, children, ...props }, ref) => (
  <Toast ref={ref} className={cn("pl-14", className)} {...props}>
    {icon && (
      <div className="absolute left-4 top-4 flex h-6 w-6 items-center justify-center rounded-full">
        {icon}
      </div>
    )}
    {children}
